                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-md-10">
                                ...
                                <table class="table table-hover">
                                    ...
                                    <tbody>
                                        @php
                                            $models = ['users' , 'roles' , 'settings' , 'learning_systems' , 'countries' , 'stages' , 'ed_classes' , 'courses' , 'chapters' , 'lessons' ,'categories' , 'orders' , 'products' , 'all_orders' , 'addresses' , 'home_page' , 'posts' , 'ads' , 'sponsers' , 'links' ,'questions' , 'withdrawals' , 'courses_orders' , 'homeworks_orders' , 'reports' , 'homeworks_monitor' , 'finances' , 'teachers' , 'homework_services' , 'wallet'];
                                            $models_ar = ['المستخدمين' , 'الصلاحيات' , 'الاعدادات' , 'الانظمة التعليمية' , 'الدول' , 'المراحل التعليمية' , 'الصفوف الدراسية' , 'الكورسات او المواد' , 'اقسام او فصول المواد' , 'الدروس' ,'اقسام المكتبة' , 'الطلبات' , 'المنتجات' , 'اضافة طلب من الادمن' , 'عنواوين التوصيل' , 'أقسام الصفحة الرئيسية' , 'الاخبار' , 'الاعلانات' , 'الداعمين' , 'روابط سريعة' ,'اسئلة الاختبارات' , 'طلبات سحب الرصيد' , 'طلبات شراء الكورسات' , 'طلبات الواجبات المدرسية' , 'الشكاوى والمقترحات' , 'متابعة الواجبات المدرسية' , 'الماليات والحسابات' , 'المعلمين' , 'خدمات حل الواجب' , 'أضافة رصيد في المحفظة'];
                                        @endphp
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                ...
                                                <select name="permissions[]"class="form-control select4  @error('permissions') is-invalid @enderror" multiple="multiple">
                                                    ...
                                                    <option value="{{$model . '-' . $permissions_map}}" {{ $role->hasPermission($model . '-' . $permissions_map) ? 'selected' : ''}}>{{ app()->getLocale() == 'ar' ? $permissions_maps_ar[$index] : $permissions_map}}</option>
                                                    @endforeach
                                                </select>
                                                @error('permissions')
                                                <span class="invalid-feedback" role="alert">
